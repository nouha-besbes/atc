...
public interface IAttendanceRepository extends IBaseRepository<Attendance, Long> {
    ...
    @Query("Select new com.atc.dto.UserAttendanceDto (a.user, CAST(a.date AS date))From Attendance a where a.isDeleted=0 and a.user.id in :usersId and CAST(a.date AS date) in :dates group by a.user.id, CAST(a.date AS date) order by a.date ASC")
